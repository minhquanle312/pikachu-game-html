...
function loadImage() {
  ...
  for (i = 1; i <= 36; i++)
    list.push(oStaticURL + "images/pieces" + i + ".jpg");
...
function newBoard() {
  ...
  div.setNextLevel = function () {
    if (this.level == 20) {
      alert(
        "Bạn đã hoàn thành tất cả 20 màn chơi! Thành tích bạn ghi được là:" +
          totalscore +
          " điểm"
      );
      this.createBigGame();
  ...
  div.changesound = function () {
    ...
    if (checksound) {
      checksound = false;
      change_soundID.val("Unmute");
      change_soundID.attr("class", "soundOff");
    } else {
      checksound = true;
      change_soundID.val("Mute");
